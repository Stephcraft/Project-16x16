...
public class EditableObject extends PClass {
	...
	protected BaseObjectData baseObjectData;
	...
	public EditableObject() {
		...
		baseObjectData = new BaseObjectData();
	...
	public void displayEdit() {
		...
		if (focus) {
			...
			applet.rect(pos.x, pos.y, baseObjectData.getWidth(), baseObjectData.getHeight());
	...
	public void updateEdit() {
		...
		// Duplicate Object Shift
			if (applet.isKeyPressEvent() && applet.keyPress(SideScroller.SHIFT)) {
				...
				switch (type) {
					case COLLISION :
						copy = new CollidableObject(baseObjectData.getId(), 0, 0);
						copy.focus = true;
						copy.focusX = focusX;
						copy.focusY = focusY;
						copy.getBaseObjectData().setPos(pos.copy());
						copy.editOffset = editOffset.copy();
						gameScene.collidableObjects.add((CollidableObject) copy);
						break;
					case OBJECT :
						try {
							Class<? extends GameObject> gameObjectClass = Tileset.getObjectClass(baseObjectData.getId());
	...
	private boolean mouseHover() {
		...
		return Util.hoverGame(pos.x, pos.y, baseObjectData.getWidth(), baseObjectData.getHeight());
	}
}
