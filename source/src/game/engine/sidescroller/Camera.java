 ...
 * game.objects or go to target position.
 * 
 * @todo deadzone mode can be choppy when tracking; zoom-to-fit (multiple
 *       game.entities); setting position to mouse when camera is rotated [bugged]
 * @author micycle1
...
public final class Camera extends ZoomPan {
	...
	public void post() {
		...
		applet.popMatrix();
		if (following) {
			PVector followObjectPos = followObject.getBaseObjectData().getPos();
			applet.rectMode(PApplet.CENTER);
			applet.rect(getCoordToDisp(followObjectPos).x, getCoordToDisp(followObjectPos).y, length * 2, length * 2);
			applet.strokeWeight(PApplet.map(PApplet.dist(applet.width / 2, applet.height / 2,
					getCoordToDisp(followObjectPos).x, getCoordToDisp(followObjectPos).y), 0, 100, 2, 10));
			applet.line(applet.width / 2, applet.height / 2, getCoordToDisp(followObjectPos).x,
					getCoordToDisp(followObjectPos).y);
			if (deadZoneScreen) {
	...
	private void update() {
		...
		if (following && ((deadZoneScreen && !withinScreenDeadZone()) || ((deadZoneWorld && !withinWorldDeadZone()))
			...
			PVector followObjectPos = followObject.getBaseObjectData().getPos();
			setPanOffset(
					PApplet.lerp(getPanOffset().x, ((-followObjectPos.x - followObjectOffset.x + offset.x) * zoom),
							lerpSpeed) - shakeOffset.x,
					PApplet.lerp(getPanOffset().y, ((-followObjectPos.y - followObjectOffset.y + offset.y) * zoom),
	...
	private boolean withinScreenDeadZone() {
		PVector coord = getCoordToDisp(followObject.getBaseObjectData().getPos()); // object screen coord
	...
	private boolean withinWorldDeadZone() {
		PVector coord = followObject.getBaseObjectData().getPos();
		return withinRegion(coord, deadZoneP1, deadZoneP2);
	}
