...
import game.objects.EditorItem;
...
//		window_test = new TestWindow(applet);
		...
		// Init Player
		player = new Player();
		player.getBaseObjectData().getPos().set(new PVector(0, -100)); // TODO: set spawn location
...
//		loadLevel(SideScroller.LEVEL); // TODO change level
	...
	/**
	 * Draw game.engine.scene elements that are below (affected by) the camera.
	 */
	public void draw() {
		background(29, 33, 45);
		...
		// View Game Objects (player-interactable game.objects)
	...
	/**
	 * Draw game.engine.scene elements that are above the camera.
	 */
	public void drawUI() {
	...
	/**
	 * Display boundaries of all world game.objects.
	 */
	public void debug() {
		...
		backgroundObjects.forEach(o -> {
			PVector objectPosition = o.getBaseObjectData().getPos();
			int objectWidth = o.getBaseObjectData().getWidth();
			int objectHeight = o.getBaseObjectData().getHeight();
			applet.rect(objectPosition.x,objectPosition.y, objectWidth, objectHeight);
		});
		...
		collidableObjects.forEach(o -> {
			PVector objectPosition = o.getBaseObjectData().getPos();
			applet.ellipse(objectPosition.x, objectPosition.y, 5, 5);
		});
		gameObjects.forEach(o -> {
			PVector objectPosition = o.getBaseObjectData().getPos();
			applet.ellipse(objectPosition.x, objectPosition.y, 5, 5);
...
//		applet.rect(applet.width / 2, 50, applet.width, 100);
	...
	/**
	 * Saves the level (background, game and collideable game.objects), encrypting the
	 * output.
	 * 
	...
	public void saveLevel(String path) {
		...
		for (int i = 0; i < collidableObjects.size(); i++) {
			...
			item.setString("id", collidableObjects.get(i).getBaseObjectData().getId());
			item.setString("type", "COLLISION");
			item.setInt("x", (int) collidableObjects.get(i).getBaseObjectData().getPos().x);
			item.setInt("y", (int) collidableObjects.get(i).getBaseObjectData().getPos().y);
			data.append(item);
		}
		...
		for (int i = 0; i < backgroundObjects.size(); i++) {
			JSONObject item = new JSONObject();
			item.setString("id", backgroundObjects.get(i).getBaseObjectData().getId());
			item.setString("type", "BACKGROUND");
			item.setInt("x", (int) backgroundObjects.get(i).getBaseObjectData().getPos().x);
			item.setInt("y", (int) backgroundObjects.get(i).getBaseObjectData().getPos().y);
			data.append(item);
		}
		...
		for (int i = 0; i < gameObjects.size(); i++) {
			...
			JSONObject item = new JSONObject();
			item.setString("id", gameObjects.get(i).getBaseObjectData().getId());
			item.setString("type", "OBJECT");
			item.setInt("x", (int) gameObjects.get(i).getBaseObjectData().getPos().x);
			item.setInt("y", (int) gameObjects.get(i).getBaseObjectData().getPos().y);
			data.append(item);
		}
	...
	public void loadLevel(String path) { // TODO save camera position/game.state.
			...
			switch (type) { // Read Main
				case "COLLISION" :
					...
					try {
						...
						collision.getBaseObjectData().setHeight(64);
						collision.getBaseObjectData().setHeight(64);
					}
					collision.getBaseObjectData().getPos().x = item.getInt("x");
					collision.getBaseObjectData().getPos().y = item.getInt("y");
				...
				case "BACKGROUND" :
					...
					backgroundObject.getBaseObjectData().getPos().x = item.getInt("x");
					backgroundObject.getBaseObjectData().getPos().y = item.getInt("y");
				...
				case "OBJECT" :
					try {
						...
						gameObject.getBaseObjectData().getPos().x = item.getInt("x");
						gameObject.getBaseObjectData().getPos().y = item.getInt("y");
