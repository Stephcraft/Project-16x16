...
public class Camera {
	...
	public void CameraInit() {
		CamMatrix.set(scale.x, 0.0f, 0, 0.0f, scale.y, 0);
	...
	public void addRotate(float degrees) {
		CamMatrix.apply(new PMatrix2D((float) Math.cos(degrees), (float) Math.sin(degrees), 0,
	...
	public void addRotateAbout(float degrees, float x, float y) {
		...
		CamMatrix.apply(new PMatrix2D((float) Math.cos(degrees), (float) Math.sin(degrees),
				x - CamMatrix.m00 * x - CamMatrix.m01 * y, -1 * (float) Math.sin(degrees), (float) Math.cos(degrees),
				y - CamMatrix.m10 * x - CamMatrix.m11 * y));
