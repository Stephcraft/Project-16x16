...
public class SceneMapEditor extends PScene {
	...
	public void draw() {
		...
		for(int i=0; i<applet.backgroundObjects.size(); i++) {
			if(tool == "MODIFY") applet.backgroundObjects.get(i).updateEdit();
		...
		for(int i=0; i<applet.collisions.size(); i++) {
			if(tool == "MODIFY") {
				...
				if(applet.collisions.get(i).focus) objectFocus = true;
			}
		...
		for(int i=0; i<applet.gameObjects.size(); i++) {
			if(tool == "MODIFY") {
				...
				if(applet.gameObjects.get(i).focus) objectFocus = true;
			}
		...
		if(tool != "INVENTORY") {
			for(int i=0; i<6; i++) {
				...
				if(applet.mousePressEvent) {
					...
					float y = 20*4/2+10;
					if(applet.mouseX > x-(20*4)/2 && applet.mouseX < x+(20*4)/2 && applet.mouseY > y-(20*4)/2 && applet.mouseY < y+(20*4)/2) {
		...
		//Update World Origin
		if(tool == "PLAY") {
			applet.originTargetX = (int)util.clamp(applet.originTargetX, applet.worldPosition.x-applet.worldWidth/2, applet.worldPosition.x+applet.worldWidth/2 - applet.width);
			applet.originTargetY = (int)util.clamp(applet.originTargetY, applet.worldPosition.y-applet.worldHeight/2, applet.worldPosition.y+applet.worldHeight/2 - applet.height);

			applet.originX = (int)util.clamp(applet.originX, applet.worldPosition.x-applet.worldWidth/2, applet.worldPosition.x+applet.worldWidth/2 - applet.width);
			applet.originY = (int)util.clamp(applet.originY, applet.worldPosition.y-applet.worldHeight/2, applet.worldPosition.y+applet.worldHeight/2 - applet.height);
		}
	}
	...
	public void displayCreativeInventory() {//complete creative inventory
		...
		for(int i=0; i<applet.gameGraphics.graphics.size(); i++) {
			...
			if(img.width > 20*4 || img.height > 20*4) {
				applet.image(img, 20*4/2+10+x*(20*4+10), y*(20*4+10) + scroll_inventory, img.width/4, img.height/4);
			}
			else {
				applet.image(img, 20*4/2+10+x*(20*4+10), y*(20*4+10) + scroll_inventory, img.width/2, img.height/2);
			}
			...
			if(applet.mousePressEvent) {
				...
				if(applet.mouseY > 100) {
					if(applet.mouseX > xx-(20*4)/2 && applet.mouseX < xx+(20*4)/2 && applet.mouseY > yy-(20*4)/2 && applet.mouseY < yy+(20*4)/2) {
						editorItem.focus = true;
		...
		// Scroll with click
		if (applet.mousePressEvent || scrollClick) {
			if (scrollClick || (applet.mouseY > 100 && applet.mouseX >= applet.width-20)) {
				scrollClick = true;
				scrollColor = 140;
				float posScroll = PApplet.map(scroll_inventory,-getInventorySize()+applet.height-8,0,applet.height-25,125);
				if (applet.mouseY < posScroll - 25) {
					scroll_inventory += 10;
				} else if (applet.mouseY > posScroll + 25){
					scroll_inventory -= 10;
				}
		...
		// Stop scrolling
		if (scrollClick && applet.mouseReleaseEvent) {
			scrollClick = false;
			scrollColor = 100;
		}
		...
		applet.fill(scrollColor);
		applet.rect(applet.width-10, PApplet.map(scroll_inventory,-getInventorySize()+applet.height-8,0,applet.height-25,125), 20, 50);
		...
		for(int i=0; i<6; i++) {
			...
			if(applet.mouseReleaseEvent) {
				...
				float yy = 20*4/2+10;
				if(editorItem.focus && applet.mouseX > xx-(20*4)/2 && applet.mouseX < xx+(20*4)/2 && applet.mouseY > yy-(20*4)/2 && applet.mouseY < yy+(20*4)/2) {
					editorItem.focus = false;
