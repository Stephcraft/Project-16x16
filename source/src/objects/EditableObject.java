...
public class EditableObject extends PClass {
	...
	public void displayEdit() {
		...
		if(focus) {
			...
			if(focusX) {
				...
				applet.line(pos.x-applet.originX, pos.y-applet.originY, pos.x+100-applet.originX, pos.y-applet.originY);
			...
			if(focusY) {
				applet.stroke(255, 213, 63);
				applet.line(pos.x-applet.originX, pos.y-applet.originY, pos.x-applet.originX, pos.y-100-applet.originY);
				...
				if(applet.keyPressEvent && applet.keyPress(16)) {
						...
						switch(type) {
							...
							case "OBJECT":
								...
								switch(id) {
								case "MIRROR_BOX":
									((MirrorBoxObject)applet.gameObjects.get(applet.gameObjects.size()-1)).direction = ((MirrorBoxObject)this).direction;
									break;
								}
	...
	private boolean mouseHoverX() {
		return (applet.mouseX > pos.x-applet.originX+100-6*4 && applet.mouseX < pos.x-applet.originX+100+6*4) &&
	...
	private boolean mouseHoverY() {
		return (applet.mouseX > pos.x-applet.originX-6*4 && applet.mouseX < pos.x-applet.originX+6*4) &&
			   (applet.mouseY > pos.y-applet.originY-100-5*4 && applet.mouseY < pos.y-applet.originY-100+5*4);
	}
	
	private boolean mouseHover() {
		if(applet.mouseX < 400 && applet.mouseY < 100) { return false; } //On Inventory Bar
		
		return (applet.mouseX > pos.x-applet.originX-width/2 && applet.mouseX < pos.x-applet.originX+width/2) &&
			   (applet.mouseY > pos.y-applet.originY-height/2 && applet.mouseY < pos.y-applet.originY+height/2);
	}
}
